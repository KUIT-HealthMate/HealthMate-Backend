...
public interface LifeStyleQuestionnaireRepository extends JpaRepository<LifeStyleQuestionnaire, Long> {
    ...
    @Query("SELECT l FROM lifestyle l WHERE l.timestamp >= :StartDate AND l.timestamp <= :EndDate")
    List<LifeStyleQuestionnaire> findByDate(LocalDateTime StartDate, LocalDateTime EndDate);
}
